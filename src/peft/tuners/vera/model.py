...
class VeraModel(LoraModel):
    """
    Creates Vera model from a pretrained transformers model.
    ...
    Example::
        ...
        >>> config = VeraConfig(
                peft_type="Vera", task_type="SEQ_2_SEQ_LM", r=8, lora_alpha=32, target_modules=["q", "v"],
                lora_dropout=0.01, train_IS=True, train_OS=True, fan_in_fan_out=True, inference_mode=False
            )
        >>> model = AutoModelForSeq2SeqLM.from_pretrained("t5-base") >>> model = VeraModel(model, config, "default")
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        # If it is not an VeraLayer, create a new module, else update it with new adapters
        if not isinstance(target, VeraLayer):
            ...
            target.update_layer(
                adapter_name,
                vera_config.r,
                vera_config.lora_alpha,
                vera_config.lora_dropout,
                train_IS=vera_config.train_IS,
                train_OS=vera_config.train_OS,
            )
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            ...
            peft_config.target_modules = TRANSFORMERS_MODELS_TO_VERA_TARGET_MODULES_MAPPING[model_config["model_type"]]
        return peft_config

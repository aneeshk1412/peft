...
from .layer import SVFTLayer, SVFTLinear
...
class SVFTModel(BaseTuner):
    ...
    Example:
        ...
        >>> from peft import SVFTConfig, get_peft_model

        >>> base_model = AutoModelForCausalLM.from_pretrained("facebook/opt-125m")
        >>> config = SVFTConfig(r=None, use_gaussian=False, svft_dropout=0.1, fan_in_fan_out=False, bias="none")
    ...
    **Attributes**:
    ...
    @staticmethod
    def _check_target_module_exists(svft_config, key):
        return check_target_module_exists(svft_config, key)
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        if isinstance(target, SVFTLinear):
            target.update_layer(
                adapter_name,
                r=svft_config.r,
                use_gaussian=svft_config.use_gaussian,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, SVFTLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _create_new_module(svft_config, adapter_name, target, **kwargs):
        ...
        if isinstance(target_base_layer, torch.nn.Linear):
            if kwargs["fan_in_fan_out"]:
                ...
                kwargs["fan_in_fan_out"] = svft_config.fan_in_fan_out = False
            ...
            if not kwargs["fan_in_fan_out"]:
                ...
                kwargs["fan_in_fan_out"] = svft_config.fan_in_fan_out = True
        else:
            raise ValueError(
                f"Target module {target} is not supported. Currently, only the following modules are supported: "
                "`torch.nn.Linear`, `transformers.pytorch_utils.Conv1D`."
            )
        new_module = SVFTLinear(
            target,
            adapter_name,
            **kwargs,
        )
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            ...
            peft_config.target_modules = set(
                TRANSFORMERS_MODELS_TO_SVFT_TARGET_MODULES_MAPPING[model_config["model_type"]]
            )
        return peft_config
    ...
    def _unload_and_optionally_merge(
        ...
        # we cannot use self.prefix as we want to include non-trainable svft parameters
    ...
    def delete_adapter(self, adapter_name: str):
        ...
        for key in key_list:
            ...
            if isinstance(target, SVFTLayer):
    ...
    def merge_and_unload(
        ...
        r"""
        This method merges the SVFT layers into the base model. This is needed if someone wants to use the base model
    ...
    def unload(self):
        """
        Gets back the base model by removing all the SVFT modules without merging. This gives back the original base
        model.
        """
